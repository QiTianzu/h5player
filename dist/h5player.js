  ...
  */
  ...
  /* 当前用到的快捷键 */
  const hasUseKey = {
    keyCodeList: [13, 16, 17, 18, 27, 32, 37, 38, 39, 40, 49, 50, 51, 52, 65, 67, 68, 69, 70, 73, 74, 75, 77, 78, 79, 80, 81, 82, 83, 84, 85, 87, 88, 89, 90, 97, 98, 99, 100, 220],
    keyList: ['enter', 'shift', 'control', 'alt', 'escape', ' ', 'arrowleft', 'arrowright', 'arrowup', 'arrowdown', '1', '2', '3', '4', 'a', 'c', 'd', 'e', 'f', 'i', 'j', 'k', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'w', 'x', 'y', 'z', '\\', '|'],
    keyMap: {
      enter: 13,
      shift: 16,
      ctrl: 17,
...
//      const mirror = t.rotateX === 180 ? `rotateX(${t.rotateX}deg)` : (t.rotateY === 180 ? `rotateY(${t.rotateY}deg)` : '');
      ...
      player.style.transform = `translate(${translate.x}px, ${translate.y}px) rotate(${t.rotate}deg) rotateX(${t.rotateX}deg) rotateY(${t.rotateY}deg)`;

      if (t.rotate === 0 || t.rotate === 180) {
        player.style.transform += ` scale(${scale})`;
      } else {
        player.style.transform += ` scale(${scale * player.videoHeight / player.videoWidth})`;
      }
    ...
    /* 视频画面旋转 ±90 度 */
    setRotate (negative = false) {
      const t = this;
      const player = t.player();
      if (negative) {
        if (t.rotateX === 180 && t.rotateY === 180) {
          t.rotate -= 270;
          t.rotateX = 0;
          t.rotateY = 0;
        } else t.rotate -=90;
        if (t.rotate < 0) t.rotate = t.rotate + 360;
      } else {
        if (t.rotateX === 180 && t.rotateY === 180) {
          t.rotate += 270;
          t.rotateX = 0;
        ...
        if (t.rotate >= 360) t.rotate = t.rotate - 360;
      }
      t.setTransform(true);
    ...
    setMirror (vertical = false) {
      ...
      if (vertical) {
        if (t.rotate === 0 || t.rotate === 180) {
          ...
          tipsMsg += ` ${i18n.t('tipsMsg.verticalMirror')} ${t.rotateY}deg`;
        }
      } else {
        if (t.rotate === 0 || t.rotate === 180) {
          t.rotateY = t.rotateY === 0 ? 180 : 0;
          ...
          t.rotateX = t.rotateX === 0 ? 180 : 0;
          tipsMsg += ` ${i18n.t('tipsMsg.horizontalMirror')} ${t.rotateX}deg`;
        }
      }
